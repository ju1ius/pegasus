...
EOS;
    ...
    static public function build()
    {
        if (null === self::$GRAMMAR) {
            self::$GRAMMAR = new self(self::SYNTAX);
    ...
    public static function getRules()
    {
        ...
        $equals = new Sequence([new Literal('=', '='), $_], 'equals');
        ...
        $not_term = new Sequence([new Literal('!'), $term, $_], 'not_term');
        array_unshift($term->members, $not_term);

        $sequence = new Sequence([$term, new OneOrMore([$term])], 'sequence');
        $or_term = new Sequence([new Literal('|'), $_, $term], 'or_term');
        $ored = new Sequence([$term, new OneOrMore([$or_term])], 'ored');
     ...
     */
    protected function _expressionsFromRules($syntax)
    {
